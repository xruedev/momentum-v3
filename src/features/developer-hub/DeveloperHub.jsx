...
export default function DeveloperHub() {
  ...
  // ── Goal helpers ───────────────────────────────────────────────
  const handleAddGoal = async (category, text) => {
    if (!text.trim() || !user) return;
    try {
      const id = generateId();
      await setDoc(doc(db, 'habits', id), {
  ...
  const handleSaveGoalEdit = async (goalId, text) => {
    if (!text.trim() || !user) return;
    try {
      await updateDoc(doc(db, 'habits', goalId), { text: text.trim(), name: text.trim() });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <section className="lg:col-span-4 bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="space-y-4">
            <GoalCategory categoryKey="liquidcars" label="Liquidcars" accent="purple" catGoals={groupedGoals.liquidcars} handleAddGoal={handleAddGoal} handleSaveGoalEdit={handleSaveGoalEdit} handleDeleteGoal={handleDeleteGoal} />
            <GoalCategory categoryKey="developer"  label="Developer"  accent="indigo"  catGoals={groupedGoals.developer}  handleAddGoal={handleAddGoal} handleSaveGoalEdit={handleSaveGoalEdit} handleDeleteGoal={handleDeleteGoal} />
          </div>
        </section>
...
const TaskList = ({ tasks, type, accentColor, placeholder, toggleTask, moveToNextDay, deleteTask, addTask }) => {
...
// ── Goal category block ────────────────────────────────────────
const GoalCategory = ({ categoryKey, label, accent, catGoals, handleAddGoal, handleSaveGoalEdit, handleDeleteGoal }) => {
  ...
  const handleAdd = async () => {
    ...
    await handleAddGoal(categoryKey, newGoalText.trim());
    setNewGoalText('');
    setIsAdding(false);
  };
  ...
  const handleSave = async (goalId) => {
    ...
    await handleSaveGoalEdit(goalId, editingGoalText.trim());
  ...
  return (
    <div className={`rounded-xl p-4 ${c.bg} border ${c.border}`}>
      <div className={`flex items-center justify-between mb-3 pb-2 border-b ${c.hb}`}>
        ...
        <button
          onClick={() => { setIsAdding(!isAdding); setNewGoalText(''); }}
          className={`p-1 rounded transition-colors border ${isAdding ? `${c.border} ${c.bg} ${c.title}` : `border-transparent ${c.addToggle}`}`}
        >
      ...
      {isAdding && (
        <div className="mb-3 flex items-center gap-2 animate-in slide-in-from-top-2 duration-200">
          ...
          />
          <button onClick={handleAdd} className={`p-1.5 ${c.addBtn} text-white rounded-lg transition-colors`}><Check className="w-3 h-3" /></button>
          <button onClick={() => setIsAdding(false)} className="p-1.5 bg-gray-200 hover:bg-gray-300 text-gray-600 rounded-lg transition-colors"><X className="w-3 h-3" /></button>
        </div>
      )}
            ...
            {editingGoalId === goal.id ? (
              <div className="flex items-center gap-2 w-full">
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && handleSave(goal.id)} autoFocus
                />
                <button onClick={() => handleSave(goal.id)} className={`p-1 ${c.addBtn} text-white rounded transition-colors`}><Check className="w-3 h-3" /></button>
